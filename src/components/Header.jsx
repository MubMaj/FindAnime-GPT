    ...
    <div className=" px-10 max-sm:px-4 bg-gradient-to-br from-black flex justify-between items-center">
      <div>
        <svg className="w-36 my-5 fill-orange-500" viewBox="0 0 136 24">
          <title id="crunchyroll">Crunchyroll logo</title>
      ...
      <div className="font-bold text-xl text-white cursor-default">
        {user?.displayName}
      </div>
      {user && (
        <button
          onClick={handleSignOut}
          className="m-2 flex items-center border border-orange-900 rounded-full bg-orange-500 text-white hover:text-orange-950 hover:bg-orange-600"
        >
          <img
            className="w-12 rounded-s-full"
            alt="usericon"
            src="https://static.crunchyroll.com/assets/avatar/170x170/egghead-luffy.png"
          />
          <span className="px-1.5 font-bold">Sign Out</span>
        </button>
      )}
    </div>
  );
